...
| det | 1,328.30 ms | 4,246.58 ms (0.31x) | 4,311.64 ms (0.31x) | SME2 Conv contention |
...
**Key insight**: KleidiAI SME2 dramatically accelerates GEMM-heavy models (rec 2.5x, textline_ori 2.7x) but the Conv-heavy det model regresses due to SME device contention on Apple M4 (2 shared devices). The net effect is positive at t=2 (pipeline 1.51x vs Paddle) because rec dominates total latency.
